...
app.get('/api/dogs', async (req,res) => {
    try {
        const[dogs] = await db.execute('SELECT d.name AS dog_name , d.size, u.username AS owner_name FROM Dogs d JOIN Users u on d.owner_id = u.user_id');
        res.json(dogs);
    } catch (err) {
...
app.get('/api/walkers/summary', async (req,res) => {
    try {
        const[dogs] = await db.execute('SELECT U.username AS walk_username, COUNT(DISTINCT WR.request_id) AS completed_walks,COUNT(R.rating_id) AS total_ratings, ROUND(AVG(R.rating),1) AS average_rating FROM Users U LEFT JOIN WalkRatings R ON U.user_id = R.walker_id LEFT JOIN WalkRequests WR ON R.request_id = WR.request_id AND WR.status = "completed" WHERE U.role = "walker" GROUP BY U.user_id');
        res.json(dogs);
    } catch (err) {
        res.status(500).json({ error: 'Failed to fetch dogs' });
    }
