...
app.get('/api/walkrequests/open', async (req,res) => {
    try {
        const[dogs] = await db.execute('SELECT WR.request_id, D.name AS dog_name, WR.requested_time, WR.duration_minutes, WR.location, U.username AS owner_username FROM WalkRequests WR JOIN Dogs D ON WR.dog_id = D.dog_id JOIN Users U ON D.');
        res.json(dogs);
    } catch (err) {
        res.status(500).json({ error: 'Failed to fetch dogs' });
    }
